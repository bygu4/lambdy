...
type Reducer (?verbose: bool) =
    ...
    let rec freeVarsCPS term cont =
        ...
        | Abstraction (Name v, term) ->
            freeVarsCPS term (fun termVars ->
                Set.remove v termVars |> cont
            )
        | Application (left, right) ->
            freeVarsCPS left (fun leftVars ->
                freeVarsCPS right (fun rightVars ->
    ...
    let nextFreeVar prefix freeVars =
        ...
        in nextFreeVar prefix freeVars
    ...
    let rec substituteCPS term (Name var, sub) cont =
        ...
        | Abstraction (Name x, term) ->
            ...
            if Set.contains var freeVarsT && Set.contains x freeVarsS then
                ...
                substituteCPS term (Name x, Variable y) (fun converted ->
                    substituteCPS converted (Name var, sub) (fun subsTerm ->
                        Abstraction (y, subsTerm) |> cont
                    )
                )
            else
                substituteCPS term (Name var, sub) (fun subsTerm ->
        ...
        | Application (left, right) ->
            substituteCPS left (Name var, sub) (fun subsLeft ->
                substituteCPS right (Name var, sub) (fun subsRight ->
