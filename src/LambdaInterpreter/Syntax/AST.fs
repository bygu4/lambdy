...
module AST =
    ...
    let rec private buildTermAST_CPS (primary: Primary.Term) cont =
        ...
        let rec buildApplicationAST (left: LambdaTerm) (right: Primary.ApplicationOpt) cont =
            ...
            | FinalAbstraction abs -> buildTermAST_CPS abs (fun right -> Application(left, right) |> cont)
            | Epsilon -> cont left
        ...
        | Primary.Abstraction(head :: tail, term) ->
            if tail.IsEmpty then
                buildTermAST_CPS term (fun termAST -> Abstraction(head, termAST) |> cont)
            else
                buildTermAST_CPS (Primary.Abstraction(tail, term)) (fun termAST -> Abstraction(head, termAST) |> cont)
        | Primary.Abstraction([], _) -> raise (ArgumentException "Abstraction received empty variable list")
    ...
    /// Get a string representation of the given lambda `term`.
    let toString (term: LambdaTerm) = toStringCPS term false true id

    /// Get a string representation of the given lambda `term` surrounded with brackets if it's not a variable.
    let toStringParenthesized (term: LambdaTerm) = toStringCPS term true true id
